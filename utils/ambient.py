...
from config import TOKEN_AMBIENT
from typing import Optional
from utils.contracts import contract_USDC, contract_ETH, contract_ambient, contract_WBTC, contract_DAI
...
class Ambient:
    ...
    async def swap_eth_to_token(self, amount: TokenAmount, contract_token: str, retry = 0, slippage = 0.1):
        print(f"{self.client.address} | Ambient Swap | {format(float(amount.Ether), '.5f')} ETH --> {self.client.name(contract_token)}...")
        logger.info(f"{self.client.address} | Ambient Swap | {format(float(amount.Ether), '.5f')} ETH --> {self.client.name(contract_token)}...")
        try:
             contract = self.client.w3.eth.contract(address=contract_ambient, abi=Ambient.abi)
    ...
    async def swap_token_to_eth(self, contract_token: str, amount: Optional[TokenAmount] = None, retry=0, slippage=0.1):
        ...
        print(f"{self.client.address} | Ambient Swap | {format(float(value.Ether), '.5f')} {self.client.name(contract_token)} --> ETH...")
        logger.info(f"{self.client.address} | Ambient Swap | {format(float(value.Ether), '.5f')} {self.client.name(contract_token)} --> ETH...")
        try:
            chek = chek_gas_eth(self.maxGas)
            if chek == False:
                return 0
